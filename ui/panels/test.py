...
class WorldViewPanel:
    ...
    def update_step_view(self):
        ...
        # 确保固定容器保持相同高度
        min_height = max(500, current_height)  # 使用保存的高度或最小高度
        self.fixed_container.configure(height=min_height)
...
"""
        ...
        print("WorldView: 准备创建AI生成模板按钮...")
        # 创建模板选择标题栏（添加水平布局来放置标题和按钮）
        template_header = ttk.Frame(scrollable_frame)
        template_header.pack(fill="x", expand=False, padx=5, pady=2)
        
        # 左侧放置标题
        ttk.Label(template_header, text="选择基础模板:").pack(side="left", anchor="w")
